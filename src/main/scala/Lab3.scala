 ...
 def eval(env: Env, e: Expr): Expr = {
   ...
   e match {
    ...
    case Binary(Plus, e1, e2) => (eToVal(e1), eToVal(e2)) match {
      ...
      case Binary(bop @ (Ne | Eq), e1, e2) => (e1, e2) match {
      ...
      case Call(func, inp) => {
        val f = eval(env,func)
        f match {
          case Function(None, param, fbody) => {
            eval(extend(env, param, eval(env, inp)), fbody)
          }
          case Function(Some(p), param, fbody) => {
            var envNew = extend(extend(env, param, eval(env, inp)), p, f)
            eval(envNew, fbody)
          }
...
def substitute(e: Expr, v: Expr, x: String): Expr = {
 ...
 e match {
   ...
   case Unary(uop, e) => Unary(uop,subst(e))
   case Binary(bop, e1, e2) => Binary(bop, subst(e1), subst(e2))
    
   case If(e1, e2, e3) => If(subst(e1), subst(e2), subst(e3))
    
   case Function(p, fx, e1) => {
     p match {
       case None => {
         if(fx==x) e
         else Function(p, fx, subst(e1))
       }
       case Some(px) => {
         if(fx==x || px==x) e
         else Function(p, fx, subst(e1))
   ...
   case Call(f, in) => Call(subst(f), subst(in))
    
   case ConstDecl(cx, e1, e2) => {
     if(cx == x) ConstDecl(cx, subst(e1), e2)
     else ConstDecl(cx, subst(e1), subst(e2))
   }
    
   case Var(vx) => if(vx==x) v else e
...
def step(e: Expr): Expr = {
  e match {
   ...
   case Unary(Neg, v1) if isValue(v1) =>N(-1*toNumber(v1)) // Do Negative
   ...
   case Binary(Plus, e1, e2) if(isValue(e1) && isValue(e2)) => (e1, e2) match {
     ...
     case Binary(And,v1, e2) if(isValue(v1) && !toBoolean(v1))=> v1
     case Binary(Or, v1, e2) if(isValue(v1) && !toBoolean(v1))=> e2
     case Binary(Or, v1, e2) if(isValue(v1) && toBoolean(v1)) => v1
     case Binary(Seq,e1, e2) if(isValue(e1)) => e2
     case Binary(bop @( Eq| Ne) ,e1, e2) if(isValue(e1)) => (e1, e2) match {
        case (Function(_,_,_),_) | (_,Function(_,_,_)) => throw DynamicTypeError(e)
        case _ if bop == Eq => B(e1==e2)
        case _ if bop == Ne => B(e1==e2)
     }
   ...
   case Call(f, e2) if(isValue(e2) && isValue(f)) => f match {
     case Function(None, x, fbody) => substitute(fbody, e2, x)
     case Function(Some(p), x, fbody) => {
       val fNewBody = substitute(fbody, e2, x)
       substitute(fNewBody, f, p)
     }
     case _ => throw DynamicTypeError(e)
   }
   ...
   case Binary(op, v1, e2) if(isValue(v1)) => (v1, e2) match {
     case (Function(_,_,_), _) | (_,Function(_,_,_)) => throw DynamicTypeError(e)
